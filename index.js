...
function CemberinCevresi(olcu) {
	return (2 * pi) * olcu;
};
console.log(CemberinCevresi(1));
...
function CemberinAlani(olcu, piDegeri) {
	return Math.pow(olcu, 2) * piDegeri;
};
console.log(CemberinAlani(2, pi));
...
for (let i = 0; i < sayilar.length - 1; i++) {
	sayilar[i] > enbuyuk ? enbuyuk = sayilar[i] : sayilar[i];
};
...
enkucuk = sayilar[0];
for (let i = 0; i < sayilar.length - 1; i++) {
	sayilar[i] < enkucuk ? enkucuk = sayilar[i] : sayilar[i];
};
...
var besyuzdenkucuksayilar = sayilar.filter(i => i < 500);
...
for(let key in liste){
	tekraredensayilar.push(`${key} sayısı ${liste[key].length} kere tekrar edilmiştir`
	)
};
console.log(tekraredensayilar, "sdfghj");
